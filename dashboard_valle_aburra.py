...
if df is not None:
    ...
    with col2:
        ...
        <div class="metric-card" style="border-left-color: #ff3f34;">
            <div class="metric-label">Casos Totales Registrados</div>
    ...
    with col4:
        ...
        <div class="metric-card" style="border-left-color: #05c46b;">
            <div class="metric-label">Casos MIS-C (SARS-CoV-2)</div>
    ...
    with tab2:
        ...
        for lag in lags_range:
            if lag == 0:
                corr = filtered_df['promedio_pm25'].corr(filtered_df['casos_totales_respiratorios'], method='spearman')
            else:
                filtered_df[f'pm25_lag_{lag}'] = filtered_df['promedio_pm25'].shift(lag)
                corr = filtered_df[f'pm25_lag_{lag}'].corr(filtered_df['casos_totales_respiratorios'], method='spearman')
        ...
        fig_corr = px.bar(
            ...
            title=f"Fuerza de Relación entre Exposición a PM2.5 y Admisión por SARI por Día de Rezago",
    ...
    # PESTAÑA 3: COMPARATIVA MUNICIPAL
    with tab3:
